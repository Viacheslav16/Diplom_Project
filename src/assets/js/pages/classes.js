...
window.new_hamb.addEventListener("click", function () {
    ...
    $(document).ready(function(){
        ...
        form.addEventListener('submit', function(e){
            ...
            const namebackFld = document.getElementById('yourbackName')
            const emailbackFld = document.getElementById('yourbackEmail')
            const addressbackFld = document.getElementById('yourbackAddress')
        
            const addbackName = namebackFld.value.trim()
            const addbackEmail = emailbackFld.value.trim()
            const addbackAddress = addressbackFld.value.trim()
            
        
            if (addbackName.value === ''){
                errors.push('Enter your name, please')
                namebackFld.classList.add('mistake')
            }else{
                if (addbackName.length < 2){
                    errors.push('Your name is too short')
                    namebackFld.classList.add('mistake')
                }
            }
            ...
            if (addbackEmail === ''){
                errors.push('Enter your email, please')
                emailbackFld.classList.add('mistake')
            }else{
                if(!isValidEmail(addbackEmail)){
                    errors.push('Incorrect email address')
                    emailbackFld.classList.add('mistake')
                }
            }
            
            if (addbackAddress.value === ''){
                errors.push('Enter your address, please')
                addressbackFld.classList.add('mistake')
            }else{
                if (addbackAddress.length < 2){
                    errors.push('Your address is too short')
                    addressbackFld.classList.add('mistake')
                }
            }
            ...
            const CHAT_ID = '-1002027278536';
            const BOT_TOKEN = '7016630485:AAHuCKIU3ORwH_w8w7clzX6rCek2PqwWcEw';
            const message = `<b>Name: </b>${addbackName}\r\n<b>Email: </b>${addbackEmail}\r\n<b>Address: </b>${addbackAddress}`
